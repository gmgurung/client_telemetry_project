...
from config import (
    ...
    S3_PROCESSED_PREFIX, SAGEMAKER_ENDPOINT_NAME,
)
...
# ---------------------------------------------------------------------------
# State / Idempotency helpers (No RDS)
# ---------------------------------------------------------------------------

def is_file_processed(s3_uri: str) -> bool:
    """
    Checks if a '.done' marker exists in S3 for the given input URI.
    URI Format: s3://bucket-name/prefix/filename.json
    """
    if not s3_uri.startswith("s3://"):
        return False  # Local files always processed for now
...
def mark_file_as_processed(s3_uri: str) -> None:
    """Creates a '.done' marker in S3 for the given input URI."""
    if not s3_uri.startswith("s3://"):
        return
    ...
    s3.put_object(
        Bucket=BUCKET_NAME, 
        Key=marker_key, 
        Body=json.dumps({"processed_at": datetime.now().isoformat(), "source": s3_uri})
    )
    print(f"  [MARKER] Created: s3://{BUCKET_NAME}/{marker_key}")
...
def run_synchronized_inference(
    ...
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    # Step 1 — Decide scoring mode first
    # ------------------------------------------------------------------
    use_endpoint = bool(endpoint_name) and _endpoint_available(endpoint_name)

    if use_endpoint:
        print(f"  [OK] Endpoint '{endpoint_name}' is InService — using endpoint scoring.")
        model_version = 0
    else:
        print("  [INFO] Endpoint unavailable or disabled — syncing local brain for fallback.")
    ...
    if update_endpoint and endpoint_name and not use_endpoint:
    ...
    # ------------------------------------------------------------------
    if is_file_processed(raw_data_path):
        print(f"  [SKIP] File already processed: {raw_data_path}")
        return

    # ------------------------------------------------------------------
    # Step 2 — Load local model artifacts only if fallback is needed
    # ------------------------------------------------------------------
    ae = iso_forest = scaler = metadata = None
    if not use_endpoint:
        ...
        print("  [OK] Local artifacts ready (AE + Isolation Forest)")
    ...
    # ------------------------------------------------------------------
    mark_file_as_processed(raw_data_path)
